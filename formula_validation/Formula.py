...
"""
  ...
  def formula_from_str_hill(formula_str: str, adduct: str, metadata: dict=None) -> 'Formula':
    ...
    if not re.search(r'^[\[?a-zA-Z0-9\]?]+(\(?[+-]?\d*\)?)?$', formula_str):
      raise IncorrectFormula(formula_str)


    pattern = r'(\[\d+\])?([A-Z][a-z]*)(\d*)'
    elements = {}
    
    for isotope, element, appearances in re.findall(pattern, formula_str):
      isotope = isotope[1:-1]if isotope else ''
      element = element + isotope if isotope else element
